...
const ListenerEthers = async ({ gasPrice }) => {
  ...
  contractController.on(filter,(address1: any,address2: any,uint256: any,int24: any,int24_2: any,OrderType: any,) => {
      const amount = BigNumber.from('80000000000000000000000');
      console.log('int24');
      console.log(int24);
      console.log(int24_2);
      generateCurrentTick({ poolAddress: address2, provider }).then(async ({token0, token1, slot0, poolContract }: any) => {
        await approveToken({address: token0,address2: ROUTER_ADDRESS,provider,wallet,amount: amount,}).then(async () => {
        await approveToken({address: token1,address2: ROUTER_ADDRESS,provider,wallet,amount: amount,});
        changeTick({token0,token1,amount: amount,desiredTick: slot0.tick,provider,wallet,poolContract,int24,int24_2,}).then(() => {
